...
- `privacy.go`:
  - `employeeEmailPattern` is now a single package-level regex matching any
    ...
    alternation over caller domains. The check no longer needs to recompile
    per call.
...
- Tests: real non-allowlisted/non-example.com email flags, `example.com`
  never flags with empty/nil config, caller allow-list domain does not flag,
...
- Kept `EmployeeEmailCheck` as the struct name (only the field inside it
  changed). It is still "the employee-email check's config," just with
...
- The `employeeEmailPattern` regex requires each DNS label be alphanumeric
  ...
  `.`-joined pair of labels. It matches a real email address's domain shape,
...
- **git-tools side**: see the matching worktree at
  ...
  own module has not been tagged or released with this change yet. git-tools
  will not compile against the new field until, first, this ai-shared-lib
  change ships a new `go/githooks` module version or tag, and second, a
  follow-up repin bumps git-tools' `go.mod` require line to it. This matches
  this repo's own established repin pattern (see recent history, e.g. "repin
  git-tools v0.9.0 to v0.10.0"). I verified the git-tools wiring compiles and
  its integration tests pass using a temporary local `replace` directive
