...
The top-level `claude-tooling` package stays stdlib-only; `tooling/codegov_lint` is the one
...
- **CI required check (SC-CODEGOV)** — `.github/workflows/codegov.yml` runs `tooling/codegov_lint` against every task's diff; see `tooling/codegov_lint/README.md`.
