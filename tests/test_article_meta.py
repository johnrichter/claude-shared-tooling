#!/usr/bin/env python3
"""Offline unit tests for article_meta.py — no network.

Verifies deterministic, verbatim extraction and null-safe degradation
...
class ExtractMetaTests(unittest.TestCase):
    ...
    def test_prefers_og_over_title_tag(self):
        """Test prefers og over title tag."""
        html = """
        <html><head>
    ...
    def test_falls_back_to_title_tag_and_time_element(self):
        """Test falls back to title tag and time element."""
        html = """
    ...
    def test_name_and_property_both_supported(self):
        """Test name and property both supported."""
        html = """
        <html><head>
    ...
    def test_all_absent_yields_all_null(self):
        """Test all absent yields all null."""
    ...
    def test_malformed_html_degrades_not_raises(self):
        ...
        """Test malformed html degrades not raises."""
        r = article_meta.extract_meta(
            "<html><head><meta property='og:title' content='X'><body><p>oops"
        )
        self.assertEqual(r["title"], "X")

    def test_published_priority_published_time_over_modified(self):
        """Test published priority published time over modified."""
        html = (
            "<html><head>"
...
class ArticleMetaFailureTests(unittest.TestCase):
    """Article meta failure tests."""

    def test_fetch_failure_returns_full_shape_all_null(self):
        # Point at an unroutable URL; fetch_html catches and returns None -> all fields null, url
        # preserved.
        """Test fetch failure returns full shape all null."""
