...
_NO_ARCHIVE = {
    "not_applicable": {
        ...
        "detail": "Prose and hooks only; consumers install it from the catalog.",
    }
}
...
def _run(*args: str, env: dict[str, str] | None = None) -> subprocess.CompletedProcess:
    return subprocess.run(
        [sys.executable, str(_CHECK), *args], capture_output=True, text=True, check=False, env=env
    )
...
class _CliCase(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        _write_json(
            ...
            {"plugins": [{"name": _NAME, "version": _VERSION}]},
        )
        _write_json(
            self.root,
    ...
    def _verify(
        self, record: Path | None = None, *args: str, env: dict[str, str] | None = None
    ) -> subprocess.CompletedProcess:
        return _run(
            "verify-release",
            "--record",
            str(record or self.record),
            "--root",
            str(self.root),
            *args,
            env=env,
...
class VerifyReleaseCliTests(_CliCase):
    ...
    def test_json_output_carries_the_verdict_and_every_enumerator(self):
        """Test json output carries the verdict and every enumerator."""
    ...
    def test_evidence_that_does_not_hold_names_the_enumerator_and_its_failure_code(self):
        """Test evidence that does not hold names the enumerator and its failure code."""
        _write_json(
            self.root,
            ".claude-plugin/marketplace.json",
    ...
    def test_a_malformed_record_is_a_usage_error(self):
        """Test a malformed record is a usage error."""
        document = self._document()
    ...
    def test_a_signed_release_verifies_end_to_end(self):
        """Test a signed release verifies end to end."""
        ...
        rendered = _run(
            ...
            f"{_ARCH}={artifact}",
            "--out",
            str(self.root / "dist" / "manifest.json"),
        )
        ...
        subprocess.run(
            [
                ...
                str(self.root / "dist" / "manifest.json"),
                "-out",
                str(self.root / "dist" / "manifest.json.sig"),
            ],
            check=True,
            capture_output=True,
        )
    ...
    def test_an_unsigned_release_fails_the_gate(self):
        """Test an unsigned release fails the gate."""
        artifact = self.root / "dist" / f"{_NAME}-{_ARCH}"
        artifact.parent.mkdir(parents=True, exist_ok=True)
        artifact.write_bytes(_PAYLOAD)
        _run(
...
class ChangedCliTests(_CliCase):
    ...
    def test_a_change_with_no_version_bump_exits_one(self):
        """Test a change with no version bump exits one."""
    ...
    def test_a_bumped_and_tagged_change_exits_zero(self):
        """Test a bumped and tagged change exits zero."""
        base = _git(self.root, "rev-parse", "HEAD")
        _write(self.root, f"{_MODULE}/README.md", "# demo\n\nNew behaviour.\n")
        _write_json(
            self.root, f"{_MODULE}/.claude-plugin/plugin.json", {"name": _NAME, "version": "1.3.0"}
        )
        _git(self.root, "add", "-A")
        ...
        self.assertEqual(result.returncode, 0, msg=result.stdout + result.stderr)
        self.assertEqual(
            [module["status"] for module in json.loads(result.stdout)["modules"]], ["released"]
        )

    def test_a_base_ref_that_does_not_resolve_is_a_usage_error(self):
        """Test a base ref that does not resolve is a usage error."""
...
class ManifestCliTests(unittest.TestCase):
    ...
    def test_a_manifest_with_no_signature_is_discarded(self):
        """Test a manifest with no signature is discarded."""
        manifest = _write(self.root, "manifest.json", self._render())
        public_key = _write(
            self.root, "pubkey.pem", "-----BEGIN PUBLIC KEY-----\nstub\n-----END PUBLIC KEY-----\n"
        )
        result = _run(
            "verify-manifest", "--manifest", str(manifest), "--public-key", str(public_key)
        )
        self.assertEqual(result.returncode, 1, msg=result.stdout + result.stderr)
    ...
    def test_a_host_with_no_verifier_reports_unverifiable(self):
        ...
        """Test a host with no verifier reports unverifiable."""
        manifest = _write(self.root, "manifest.json", self._render())
        ...
        env = {**os.environ, "PATH": ""}
        result = _run(
            "verify-manifest", "--manifest", str(manifest), "--public-key", str(public_key), env=env
        )
...
class ProvisionCliTests(unittest.TestCase):
    ...
    def test_nothing_provisionable_exits_three(self):
        """Test nothing provisionable exits three."""
        result = _run(
            "provision",
            "--name",
            _NAME,
    ...
    def test_a_version_keyed_cache_entry_is_reverified_and_returned(self):
        """Test a version keyed cache entry is reverified and returned."""
        ...
        digest = subprocess.run(
            [
                ...
                "-c",
                "import "
                "hashlib,sys;print(hashlib.sha256(open(sys.argv[1],'rb').read()).hexdigest())",
                str(cached),
            ],
            check=True,
            capture_output=True,
