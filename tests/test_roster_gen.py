...
from roster_gen import render  # noqa: E402
...
def _seed_patch_targets(roots: dict[str, Path]) -> None:
    """Pre-create every target's parent directory and seed "patch" target content.

    Parent directories are created at each target's real rel_path under the fixture roots
    (targets.py hardcodes those paths, so a fixture must mirror them). A "whole" or
    "allowlist" target has no such prerequisite — generate.py replaces or creates it
    outright.
    """
...
class RenderSpecsTests(unittest.TestCase):
    ...
    def test_idempotent_and_deterministic_order(self) -> None:
        """Test idempotent and deterministic order."""
        models = {
        ...
        reordered = render.render_specs(
            _roster({"a-model": models["a-model"], "z-model": models["z-model"]}), "v1"
        )
        self.assertEqual(first, reordered)
        doc = json.loads(first)
        self.assertEqual(
            list(doc["model"].keys()), ["a-model", "z-model"]
        )  # opus (rank 9) before haiku (rank 0)

    def test_generated_by_header_names_tag(self) -> None:
        """Test generated by header names tag."""
...
class PlanSchemaEnumTests(unittest.TestCase):
    """Plan schema enum tests."""

    def test_enum_is_new_work_plus_sentinels_only(self) -> None:
        """Test enum is new work plus sentinels only."""
        models = {
    ...
    def test_unlisted_id_appended_not_dropped(self) -> None:
        """Test unlisted id appended not dropped."""
    ...
    def test_missing_required_sentinel_fails_loud(self) -> None:
        """The guard checks a fixed requirement, not the roster's own list parroted back.

        A roster that drops 'inherit' from effort_exempt_sentinels must still fail.
        """
    ...
    def test_generated_by_comment_names_tag(self) -> None:
        """Test generated by comment names tag."""
...
class GateAllowlistTests(unittest.TestCase):
    """Gate allowlist tests."""

    def test_excludes_retired_includes_legacy_pin_only(self) -> None:
        """Test excludes retired includes legacy pin only."""
        models = {
            "claude-sonnet-5": _model(selectable="new-work"),
            "claude-opus-4-5": _model(selectable="legacy-pin-only"),
            "claude-retired-1": _model(selectable="retired"),
    ...
    def test_plan_enum_and_gate_allowlist_are_distinct_projections(self) -> None:
        ...
        allowlist_ids = {
            ln
            for ln in render.render_gate_allowlist(roster, "v1").splitlines()
    ...
    def test_narrowing_an_already_allowed_id_fails(self) -> None:
        """Test narrowing an already allowed id fails."""
...
class BuildEngineAndTieringDocTests(unittest.TestCase):
    """Build engine and tiering doc tests."""

    def test_build_engine_bootstrap_then_idempotent_regenerate(self) -> None:
        """Test build engine bootstrap then idempotent regenerate."""
    ...
    def test_tiering_doc_bootstrap_then_idempotent_regenerate(self) -> None:
        """Test tiering doc bootstrap then idempotent regenerate."""
...
class PlantedRowChangeScopingTests(unittest.TestCase):
    ...
    def test_price_change_reaches_specs_and_rates_not_enum_or_allowlist(self) -> None:
        """Test price change reaches specs and rates not enum or allowlist."""
        ...
        rates_before = render.patch_build_engine(
            _BOOTSTRAP_BUILD_ENGINE, _roster(base_models), "v1"
        )
        rates_after = render.patch_build_engine(
            _BOOTSTRAP_BUILD_ENGINE, _roster(changed_models), "v1"
        )
    ...
    def test_selectable_change_reaches_enum_and_tiering_doc_not_specs_or_rates(self) -> None:
        """Test selectable change reaches enum and tiering doc not specs or rates."""
...
def _with_1m_variant(model: dict, *, list_output: float, window: int = 1000000) -> dict:
    """`model` given a `context_variants['1m']` block with its own window and list price.

    Distinct from the base row's, so a test can prove the projection reads the variant, not
    the base.
    """
    return {
        **model,
...
class ContextVariantProjectionTests(unittest.TestCase):
    """The `<id>[1m]` window-variant projection.

    Emitted for every row that declares a `1m` context variant, from that row's own variant
    data, never hand-added; appended as a trailing block (never interleaved with base rows);
    the base row keeps its own window.
    """

    def test_specs_project_suffixed_entry_from_variant_data(self) -> None:
        """Test specs project suffixed entry from variant data."""
        ...
        self.assertEqual(
            doc["model"]["claude-sonnet-5"]["context_window"], 200000
        )  # base untouched
    ...
    def test_specs_row_without_variant_gets_no_suffixed_entry(self) -> None:
        """Test specs row without variant gets no suffixed entry."""
    ...
    def test_specs_variants_are_a_trailing_block_never_interleaved(self) -> None:
        """Test specs variants are a trailing block never interleaved."""
        models = {
            "claude-opus-5": _with_1m_variant(
                _model(family="opus", cross_family_rank=9), list_output=25.0
            ),
    ...
    def test_plan_enum_comment_accounts_for_variant_pin(self) -> None:
        """Test plan enum comment accounts for variant pin."""
        out = render.patch_plan_schema(
            _BOOTSTRAP_PLAN_SCHEMA,
            _roster({"claude-sonnet-5": _with_1m_variant(_model(), list_output=15.0)}),
            "v1",
        )
        comment = json.loads(out)["$defs"]["task"]["properties"]["model"]["$comment"]
    ...
    def test_tiering_table_marks_a_row_with_a_1m_variant(self) -> None:
        """Test tiering table marks a row with a 1m variant."""
        ...
        without = render.patch_tiering_doc(
            _BOOTSTRAP_TIERING_DOC, _roster({"claude-sonnet-5": _model()}), "v1"
        )
...
class RosterLoadTests(unittest.TestCase):
    ...
    def test_forward_schema_version_refused(self) -> None:
        """Test forward schema version refused."""
    ...
    def test_missing_effort_exempt_sentinels_refused(self) -> None:
        """Test missing effort exempt sentinels refused."""
        with tempfile.TemporaryDirectory() as tmp:
...
class CliIntegrationTests(unittest.TestCase):
    """Drive generate.py as a subprocess against throwaway two-root temp trees.

    Exactly as a real invocation would (README's --ai-shared-lib-root / --marketplace-root
    split).
    """
    ...
    def setUp(self) -> None:
        ...
        self.roots = {
            "ai-shared-lib": self.ai_shared_lib_root,
            "marketplace": self.marketplace_root,
        }
    ...
    def _all_target_texts(self) -> dict[str, str]:
        return {
            f"{t.repo}:{t.rel_path}": resolve(t, self.roots).read_text(encoding="utf-8")
            for t in TARGETS
        }
    ...
    def test_generate_twice_is_byte_identical(self) -> None:
        ...
        """Test generate twice is byte identical."""
    ...
    def test_generated_by_header_names_tag_in_every_target(self) -> None:
        """Test generated by header names tag in every target."""
        self._write_roster({"claude-sonnet-5": _model()})
    ...
    def test_check_passes_on_a_clean_tree_then_catches_a_hand_edit(self) -> None:
        """Test check passes on a clean tree then catches a hand edit."""
        self._write_roster({"claude-sonnet-5": _model()})
    ...
    def test_check_fails_when_a_patch_target_is_absent(self) -> None:
        """Test check fails when a patch target is absent."""
        self._write_roster({"claude-sonnet-5": _model()})
        self.assertEqual(self._run("generate").returncode, 0)
    ...
    def test_roster_change_with_no_regeneration_fails_check(self) -> None:
        """Test roster change with no regeneration fails check."""
    ...
    def test_retiring_an_allowed_id_fails_generate_with_no_partial_write(self) -> None:
        """Test retiring an allowed id fails generate with no partial write."""
...
class RealRepoByteParityTests(unittest.TestCase):
    """Opportunistic integration check with both repos checked out side by side.

    In this generator's real execution context (per its README), every committed output must
    already equal its roster-derived rendering at its own recorded tag.
    """

    def test_every_target_matches_its_own_recorded_tag(self) -> None:
        """Test every target matches its own recorded tag."""
