...
def _publish(directory: Path, *, name: str, version: str, arch: str, payload: bytes) -> str:
    ...
    manifest = provisioning.render_manifest(
        version, [provisioning.Artifact.from_path(arch, artifact)]
    )
...
class _ReleaseCase(unittest.TestCase):
    ...
    def _build_tree(self, version: str) -> None:
        ...
        _write_json(
            self.root, f"{_MODULE}/.claude-plugin/plugin.json", {"name": _NAME, "version": version}
        )
        _write(self.root, f"{_MODULE}/CHANGELOG.md", f"# Changelog\n\n## {version}\n\n- released\n")
        _write_json(
            self.root,
            ".claude-plugin/marketplace.json",
            {"plugins": [{"name": _NAME, "source": f"./{_MODULE}", "version": version}]},
        )
        _write_json(
            self.root,
            ".claude/settings.json",
            {"enabledPlugins": {f"{_NAME}@{_MARKETPLACE}": True}},
        )
        ...
        _write(
            self.root,
            "dist/manifest.json",
            provisioning.render_manifest(
                version, [provisioning.Artifact.from_path(_ARCH, artifact)]
            ),
        )
    ...
    def _evaluate(
        self, record, *, verifier=_accepts, pause_register: Path | None = None, contract=None
    ):
        return evidence.evaluate(
            contract or self.contract,
            record,
            self.root,
            pause_register=pause_register,
            verifier=verifier,
        )
    ...
    def _result(self, transaction, enumerator_id: str):
        ...
        matches = [
            result for result in transaction.results if result.enumerator_id == enumerator_id
        ]
        self.assertEqual(
            len(matches),
            1,
            msg=f"{enumerator_id}: {[r.enumerator_id for r in transaction.results]}",
        )
        return matches[0]
...
class EnumeratorSetTests(unittest.TestCase):
    ...
    def test_the_contract_pins_the_seven_enumerators_in_order(self):
        """Test the contract pins the seven enumerators in order."""
    ...
    def test_every_declared_evidence_kind_has_exactly_one_resolver(self):
        ...
        """Test every declared evidence kind has exactly one resolver."""
    ...
    def test_every_enumerator_names_a_declared_kind_and_a_missing_message(self):
        """Test every enumerator names a declared kind and a missing message."""
    ...
    def test_only_the_waivable_enumerators_carry_reasons(self):
        """Test only the waivable enumerators carry reasons."""
        waivable = {
            enumerator.id
            for enumerator in self.contract.enumerators
            if enumerator.not_applicable_reasons
        }
        self.assertEqual(
            waivable, {"catalog_entry", "enable_state", "artifacts", "download_script_pin"}
        )

    def test_a_duplicated_enumerator_id_is_rejected(self):
        """Test a duplicated enumerator id is rejected."""
    ...
    def test_an_empty_enumerator_list_is_rejected(self):
        """Test an empty enumerator list is rejected."""
        document = json.loads(CONTRACT_PATH.read_text(encoding="utf-8"))
    ...
    def test_the_record_schema_does_not_restate_the_enumerator_ids(self):
        ...
        """Test the record schema does not restate the enumerator ids."""
        schema = json.loads(
            (CONTRACT_DIR / "release-transaction-record.schema.json").read_text(encoding="utf-8")
        )
...
class CompleteReleaseTests(_ReleaseCase):
    ...
    def test_a_complete_release_passes(self):
        ...
        self.assertEqual(
            [result.status for result in transaction.results],
            [evidence.Status.SATISFIED] * len(_ENUMERATORS),
        )

    def test_results_follow_the_contract_order(self):
        """Test results follow the contract order."""
        transaction = self._evaluate(self._record())
        self.assertEqual(
            tuple(result.enumerator_id for result in transaction.results),
            self.contract.enumerator_ids,
        )

    def test_each_missing_enumerator_fails_naming_itself(self):
        """Test each missing enumerator fails naming itself."""
        for enumerator_id in _ENUMERATORS:
            with self.subTest(enumerator=enumerator_id):
                ...
                self.assertIn(
                    enumerator_id, [failure.enumerator_id for failure in transaction.failures]
                )
    ...
    def test_an_unbound_version_also_fails_the_tag_it_anchors(self):
        ...
        """Test an unbound version also fails the tag it anchors."""
        transaction = self._evaluate(self._record(drop="version"))
        self.assertEqual(
            [failure.enumerator_id for failure in transaction.failures], ["version", "tag"]
        )
    ...
    def test_only_the_dropped_enumerator_fails(self):
        """Test only the dropped enumerator fails."""
        for enumerator_id in [name for name in _ENUMERATORS if name != "version"]:
            with self.subTest(enumerator=enumerator_id):
                transaction = self._evaluate(self._record(drop=enumerator_id))
                self.assertEqual(
                    [failure.enumerator_id for failure in transaction.failures], [enumerator_id]
                )
    ...
    def test_the_gate_demands_whatever_the_contract_declares(self):
        ...
        """Test the gate demands whatever the contract declares."""
        document = json.loads(CONTRACT_PATH.read_text(encoding="utf-8"))
        ...
        self.assertEqual(
            [failure.enumerator_id for failure in transaction.failures], ["attestation"]
        )
    ...
    def test_a_record_naming_an_undeclared_enumerator_is_rejected(self):
        """Test a record naming an undeclared enumerator is rejected."""
    ...
    def test_a_binding_missing_a_required_parameter_is_rejected(self):
        """Test a binding missing a required parameter is rejected."""
        with self.assertRaises(RecordError) as raised:
    ...
    def test_a_binding_with_an_undeclared_parameter_is_rejected(self):
        """Test a binding with an undeclared parameter is rejected."""
        with self.assertRaises(RecordError) as raised:
            self._record(
                bindings={"changelog": {"path": f"{_MODULE}/CHANGELOG.md", "grep": "1.2.3"}}
            )
...
class EvidenceDriftTests(_ReleaseCase):
    ...
    def test_a_module_manifest_at_another_version_fails(self):
        """Test a module manifest at another version fails."""
        _write_json(
            self.root, f"{_MODULE}/.claude-plugin/plugin.json", {"name": _NAME, "version": "1.2.2"}
        )
        self.assertEqual(
            self._result(self._evaluate(self._record()), "version").code, "value-mismatch"
        )

    def test_a_catalog_lagging_the_plugin_fails(self):
        """Test a catalog lagging the plugin fails."""
        _write_json(
            self.root,
            ".claude-plugin/marketplace.json",
            {"plugins": [{"name": _NAME, "version": "1.2.2"}]},
        )
        self.assertEqual(
            self._result(self._evaluate(self._record()), "catalog_entry").code, "value-mismatch"
        )

    def test_a_catalog_with_no_entry_for_the_module_fails(self):
        """Test a catalog with no entry for the module fails."""
        _write_json(
            self.root,
            ".claude-plugin/marketplace.json",
            {"plugins": [{"name": "other", "version": _VERSION}]},
        )
        self.assertEqual(
            self._result(self._evaluate(self._record()), "catalog_entry").code, "entry-absent"
        )

    def test_a_plugin_nobody_enables_fails(self):
        """Test a plugin nobody enables fails."""
        _write_json(
            self.root,
            ".claude/settings.json",
            {"enabledPlugins": {f"{_NAME}@{_MARKETPLACE}": False}},
        )
        self.assertEqual(
            self._result(self._evaluate(self._record()), "enable_state").code, "value-mismatch"
        )

    def test_an_absent_enable_state_key_fails(self):
        """Test an absent enable state key fails."""
        _write_json(self.root, ".claude/settings.json", {"enabledPlugins": {}})
        self.assertEqual(
            self._result(self._evaluate(self._record()), "enable_state").code, "key-absent"
        )

    def test_a_tag_off_the_convention_fails(self):
        """Test a tag off the convention fails."""
        result = self._result(
            self._evaluate(self._record(bindings={"tag": {"tag": "release-2024-06"}})), "tag"
        )
        self.assertEqual(result.code, "tag-off-convention")

    def test_a_conventional_tag_that_does_not_exist_fails(self):
        """Test a conventional tag that does not exist fails."""
        result = self._result(
            self._evaluate(self._record(bindings={"tag": {"tag": f"{_NAME}-v{_VERSION}"}})), "tag"
        )
        self.assertEqual(result.code, "tag-absent")

    def test_a_tag_pointing_at_a_pre_bump_tree_fails(self):
        """Test a tag pointing at a pre bump tree fails."""
    ...
    def test_a_pin_left_on_the_previous_version_fails(self):
        """Test a pin left on the previous version fails."""
        _write(self.root, "scripts/install.sh", '#!/bin/sh\nVERSION="1.2.2"\n')
        self.assertEqual(
            self._result(self._evaluate(self._record()), "download_script_pin").code,
            "value-mismatch",
        )

    def test_a_script_pinning_two_versions_fails(self):
        """Test a script pinning two versions fails."""
        _write(
            self.root,
            "scripts/install.sh",
    ...
    def test_a_changelog_without_the_released_version_fails(self):
        """Test a changelog without the released version fails."""
        _write(self.root, f"{_MODULE}/CHANGELOG.md", "# Changelog\n\n## 1.2.2\n\n- previous\n")
        self.assertEqual(
            self._result(self._evaluate(self._record()), "changelog").code, "entry-absent"
        )

    def test_an_absent_evidence_file_fails_rather_than_passing(self):
        """Test an absent evidence file fails rather than passing."""
        (self.root / ".claude-plugin" / "marketplace.json").unlink()
        self.assertEqual(
            self._result(self._evaluate(self._record()), "catalog_entry").code, "file-absent"
        )
...
class WaiverTests(_ReleaseCase):
    ...
    _WAIVER = {
        "not_applicable": {
            "reason": "no-catalog",
            "detail": "Released as a library; no catalog publishes it.",
        }
    }

    def test_a_declared_waiver_with_an_allowed_reason_passes(self):
        """Test a declared waiver with an allowed reason passes."""
        transaction = self._evaluate(self._record(bindings={"catalog_entry": self._WAIVER}))
        self.assertEqual(
            self._result(transaction, "catalog_entry").status, evidence.Status.NOT_APPLICABLE
        )
        self.assertEqual(transaction.verdict, "pass")

    def test_a_reason_the_contract_does_not_allow_here_fails(self):
        """Test a reason the contract does not allow here fails."""
        binding = {
            "not_applicable": {
                "reason": "no-release-archive",
                "detail": "Borrowed from another enumerator.",
            }
        }
        result = self._result(
            self._evaluate(self._record(bindings={"catalog_entry": binding})), "catalog_entry"
        )
    ...
    def test_a_non_waivable_enumerator_cannot_be_waived(self):
        """Test a non waivable enumerator cannot be waived."""
        for enumerator_id in ("version", "tag", "changelog"):
            with self.subTest(enumerator=enumerator_id):
                binding = {"not_applicable": {"reason": "no-catalog", "detail": "Arguing it away."}}
                result = self._result(
                    self._evaluate(self._record(bindings={enumerator_id: binding})), enumerator_id
                )
                self.assertEqual(result.code, "waiver-not-permitted")
                self.assertEqual(result.status, evidence.Status.UNSATISFIED)

    def test_a_waiver_mixed_with_evidence_is_rejected(self):
        """Test a waiver mixed with evidence is rejected."""
        binding = {
            "not_applicable": {"reason": "no-catalog", "detail": "Both at once."},
    ...
    def test_a_waiver_without_a_human_statement_is_rejected(self):
        """Test a waiver without a human statement is rejected."""
        with self.assertRaises(RecordError):
            self._record(
                bindings={
                    "catalog_entry": {"not_applicable": {"reason": "no-catalog", "detail": "  "}}
                }
            )
...
class ManifestTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.manifest.write_text(
            provisioning.render_manifest(_VERSION, [self.entry]), encoding="utf-8"
        )
    ...
    def test_rendering_is_one_deterministic_pass(self):
        """Test rendering is one deterministic pass."""
        other = provisioning.Artifact(
            arch="darwin-aarch64", filename="demo-darwin-aarch64", sha256="a" * 64, size=7
        )
    ...
    def test_a_post_edited_manifest_is_not_canonical(self):
        """Test a post edited manifest is not canonical."""
        document = json.loads(self.manifest.read_text(encoding="utf-8"))
        self.assertFalse(provisioning.is_canonical(json.dumps(document, indent=4) + "\n"))
        self.assertFalse(
            provisioning.is_canonical(self.manifest.read_text(encoding="utf-8") + "\n")
        )

    def test_a_missing_signature_discards_the_manifest(self):
        """Test a missing signature discards the manifest."""
        self.signature.unlink()
        verification = provisioning.verify_manifest(
            self.manifest, self.signature, self.public_key, verifier=_accepts
        )
    ...
    def test_an_invalid_signature_discards_the_manifest(self):
        """Test an invalid signature discards the manifest."""
        verification = provisioning.verify_manifest(
            self.manifest, self.signature, self.public_key, verifier=_rejects
        )
        self.assertEqual(verification.verdict, provisioning.SignatureVerdict.DISCARDED)
    ...
    def test_a_missing_signature_discards_even_where_no_verifier_exists(self):
        ...
        """Test a missing signature discards even where no verifier exists."""
        self.signature.unlink()
        verification = provisioning.verify_manifest(
            self.manifest, self.signature, self.public_key, verifier=_cannot_verify
        )
        self.assertEqual(verification.verdict, provisioning.SignatureVerdict.DISCARDED)
        self.assertEqual(verification.code, "signature-missing")

    def test_a_host_with_no_verifier_is_unverifiable(self):
        """Test a host with no verifier is unverifiable."""
        verification = provisioning.verify_manifest(
            self.manifest, self.signature, self.public_key, verifier=_cannot_verify
        )
    ...
    def test_an_absent_public_key_is_named_apart_from_an_absent_verifier(self):
        """Test an absent public key is named apart from an absent verifier."""
    ...
    def test_an_authentic_manifest_that_is_not_manifest_shaped_is_discarded(self):
        """Test an authentic manifest that is not manifest shaped is discarded."""
    ...
    def test_artifact_bytes_are_checked_against_the_recorded_digest_and_size(self):
        """Test artifact bytes are checked against the recorded digest and size."""
    ...
    def test_a_plaintext_transport_is_refused(self):
        """Test a plaintext transport is refused."""
        self.assertIsNotNone(
            provisioning.fetch("http://example.invalid/manifest.json", self.root / "fetched.json")
        )
    ...
    def test_openssl_round_trip_verifies_and_rejects(self):
        """Test openssl round trip verifies and rejects."""
        ...
        subprocess.run(
            ["openssl", "pkey", "-in", str(private_key), "-pubout", "-out", str(self.public_key)],
            check=True,
            capture_output=True,
        )
        subprocess.run(
        ...
        self.assertEqual(
            verification.verdict, provisioning.SignatureVerdict.VERIFIED, msg=verification.detail
        )
        self.assertIsNotNone(verification.manifest)

        self.manifest.write_text(
            provisioning.render_manifest("9.9.9", [self.entry]), encoding="utf-8"
        )
...
class ArtifactEnumeratorTests(_ReleaseCase):
    ...
    def test_a_missing_signature_fails_the_gate_even_though_every_digest_matches(self):
        ...
        """Test a missing signature fails the gate even though every digest matches."""
    ...
    def test_an_invalid_signature_fails_the_gate(self):
        """Test an invalid signature fails the gate."""
    ...
    def test_a_host_that_cannot_verify_fails_the_gate(self):
        """Test a host that cannot verify fails the gate."""
    ...
    def test_a_post_edited_manifest_fails_the_gate(self):
        """Test a post edited manifest fails the gate."""
        ...
        self.assertEqual(
            self._result(self._evaluate(self._record()), "artifacts").code, "manifest-non-canonical"
        )

    def test_a_manifest_at_another_version_fails(self):
        """Test a manifest at another version fails."""
        artifact = self.root / "dist" / f"{_NAME}-{_ARCH}"
        _write(
            self.root,
            "dist/manifest.json",
            provisioning.render_manifest(
                "1.2.2", [provisioning.Artifact.from_path(_ARCH, artifact)]
            ),
        )
        self.assertEqual(
            self._result(self._evaluate(self._record()), "artifacts").code, "version-mismatch"
        )
    ...
    def test_a_tampered_artifact_fails(self):
        ...
        self.assertEqual(
            self._result(self._evaluate(self._record()), "artifacts").code, "artifact-mismatch"
        )

    def test_a_required_arch_with_no_entry_fails(self):
        """Test a required arch with no entry fails."""
        bindings = {
            "artifacts": {
                "manifest": "dist/manifest.json",
                "public_key": "release/pubkey.pem",
                "require_arches": ["darwin-aarch64"],
            }
        }
        self.assertEqual(
            self._result(self._evaluate(self._record(bindings=bindings)), "artifacts").code,
            "arch-absent",
        )
...
class LadderTests(unittest.TestCase):
    ...
    def _provision(
        self, *, base_url: str | None = None, frozen_dir: Path | None = None, verifier=_accepts
    ):
    ...
    def _seed_cache(self, payload: bytes, *, digest: str | None = None) -> Path:
        ...
        Path(f"{cached}.sha256").write_text(
            f"{digest or provisioning.sha256_of(cached)}\n", encoding="utf-8"
        )
        return cached
    ...
    def test_a_version_keyed_cache_entry_is_reverified_in_place(self):
        """Test a version keyed cache entry is reverified in place."""
    ...
    def test_a_cache_entry_failing_reverification_is_evicted(self):
        """Test a cache entry failing reverification is evicted."""
    ...
    def test_a_cache_entry_without_a_sidecar_is_evicted(self):
        """Test a cache entry without a sidecar is evicted."""
        cached = self.cache / f"{_NAME}-{_VERSION}"
    ...
    def test_the_current_release_is_preferred_over_a_commit_frozen_copy(self):
        """Test the current release is preferred over a commit frozen copy."""
        base_url = _publish(
            self.root / "releases", name=_NAME, version=_VERSION, arch=_ARCH, payload=_PAYLOAD
        )
    ...
    def test_a_resolved_release_is_cached_with_its_sidecar(self):
        """Test a resolved release is cached with its sidecar."""
        base_url = _publish(
            self.root / "releases", name=_NAME, version=_VERSION, arch=_ARCH, payload=_PAYLOAD
        )
        ...
        self.assertEqual(
            sidecar.read_text(encoding="utf-8").split()[0], provisioning.sha256_of(result.path)
        )

    def test_a_discarded_release_manifest_stops_the_ladder(self):
        """Test a discarded release manifest stops the ladder."""
        base_url = _publish(
            self.root / "releases", name=_NAME, version=_VERSION, arch=_ARCH, payload=_PAYLOAD
        )
        frozen = _freeze(self.root / "frozen", name=_NAME, arch=_ARCH, payload=_FROZEN_PAYLOAD)
    ...
    def test_an_unverifiable_release_walks_on_to_the_frozen_copy(self):
        """Test an unverifiable release walks on to the frozen copy."""
        base_url = _publish(
            self.root / "releases", name=_NAME, version=_VERSION, arch=_ARCH, payload=_PAYLOAD
        )
        frozen = _freeze(self.root / "frozen", name=_NAME, arch=_ARCH, payload=_FROZEN_PAYLOAD)
        result = self._provision(
            base_url=base_url, frozen_dir=frozen.parent, verifier=_cannot_verify
        )
    ...
    def test_an_unreachable_release_walks_on_to_the_frozen_copy(self):
        """Test an unreachable release walks on to the frozen copy."""
        frozen = _freeze(self.root / "frozen", name=_NAME, arch=_ARCH, payload=_FROZEN_PAYLOAD)
        result = self._provision(
            base_url=(self.root / "no-releases").as_uri(), frozen_dir=frozen.parent
        )
        self.assertIs(result.rung, provisioning.Rung.FROZEN)

    def test_a_frozen_copy_without_a_sidecar_is_rejected(self):
        """Test a frozen copy without a sidecar is rejected."""
    ...
    def test_an_unmaterialized_large_file_pointer_counts_as_absent(self):
        """Test an unmaterialized large file pointer counts as absent."""
        frozen_dir = self.root / "frozen"
        frozen_dir.mkdir()
        ...
        Path(f"{pointer}.sha256").write_text(
            f"{provisioning.sha256_of(pointer)}\n", encoding="utf-8"
        )
        result = self._provision(frozen_dir=frozen_dir)
        self.assertFalse(result.provisioned)
    ...
    def test_nothing_available_resolves_nothing(self):
        """Test nothing available resolves nothing."""
        result = self._provision()
        self.assertFalse(result.provisioned)
        self.assertIs(result.rung, provisioning.Rung.NONE)
...
class ChangedModuleTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        _init_repo(self.root)
        _write_json(
            self.root, f"{_MODULE}/.claude-plugin/plugin.json", {"name": _NAME, "version": _VERSION}
        )
    ...
    def test_a_change_with_no_version_bump_fails(self):
        """Test a change with no version bump fails."""
        _write(self.root, f"{_MODULE}/README.md", "# demo\n\nNew behaviour.\n")
    ...
    def test_a_bump_with_no_published_release_fails(self):
        """Test a bump with no published release fails."""
        _write(self.root, f"{_MODULE}/README.md", "# demo\n\nNew behaviour.\n")
        self._bump("1.3.0")
    ...
    def test_a_bump_with_a_release_tag_passes(self):
        """Test a bump with a release tag passes."""
        _write(self.root, f"{_MODULE}/README.md", "# demo\n\nNew behaviour.\n")
        self._bump("1.3.0")
    ...
    def test_a_version_that_does_not_increase_fails(self):
        """Test a version that does not increase fails."""
        _write(self.root, f"{_MODULE}/README.md", "# demo\n\nNew behaviour.\n")
        self._bump("1.2.2")
        _commit(self.root, "walk the version backwards")
        self.assertEqual(
            [verdict.status for verdict in self._check().verdicts], ["version-not-increased"]
        )

    def test_a_prerelease_suffix_alone_is_not_a_bump(self):
        """Test a prerelease suffix alone is not a bump."""
        _write(self.root, f"{_MODULE}/README.md", "# demo\n\nNew behaviour.\n")
    ...
    def test_repository_infrastructure_ships_in_no_release(self):
        """Test repository infrastructure ships in no release."""
        _write(self.root, ".github/workflows/ci.yml", "name: CI\n\n# tweak\n")
    ...
    def test_a_path_under_no_module_belongs_to_no_release(self):
        """Test a path under no module belongs to no release."""
        _write(self.root, "notes.md", "scratch\n\nmore\n")
    ...
    def test_a_tag_versioned_module_with_no_tag_after_the_base_fails(self):
        """Test a tag versioned module with no tag after the base fails."""
    ...
    def test_a_tag_versioned_module_released_after_the_base_passes(self):
        """Test a tag versioned module released after the base passes."""
        _write_json(self.root, "schemas/thing/thing.schema.json", {"title": "Thing"})
        _commit(self.root, "add a contract module")
    ...
    def test_a_base_ref_that_does_not_resolve_is_an_error(self):
        """Test a base ref that does not resolve is an error."""
    ...
    def test_a_directory_that_is_not_a_repository_is_an_error(self):
        """Test a directory that is not a repository is an error."""
...
class ReleasePauseTests(_ReleaseCase):
    ...
    def _register(self, **overrides) -> Path:
        ...
        return _write_json(
            ...
            {"schema": "release-pause-register@1.0.0", "entries": [entry]},
        )

    def test_the_hook_names_the_consumer_that_writes_the_register(self):
        """Test the hook names the consumer that writes the register."""
    ...
    def test_an_open_defect_pauses_an_otherwise_complete_release(self):
        """Test an open defect pauses an otherwise complete release."""
    ...
    def test_an_absent_register_pauses_nothing(self):
        """Test an absent register pauses nothing."""
        transaction = self._evaluate(
            self._record(), pause_register=self.root / "no-such-register.json"
        )
        self.assertEqual(transaction.verdict, "pass")

    def test_a_resolved_or_unmeasured_entry_pauses_nothing(self):
        """Test a resolved or unmeasured entry pauses nothing."""
        for status in ("resolved", "declared-unmeasured"):
            with self.subTest(status=status):
                transaction = self._evaluate(
                    self._record(), pause_register=self._register(status=status)
                )
                self.assertEqual(transaction.verdict, "pass")

    def test_a_defect_against_another_module_does_not_pause_this_release(self):
        """Test a defect against another module does not pause this release."""
        transaction = self._evaluate(
            self._record(), pause_register=self._register(owner="other-plugin")
        )
        self.assertEqual(transaction.verdict, "pass")

    def test_a_partial_release_reports_the_partial_rather_than_the_pause(self):
        """Test a partial release reports the partial rather than the pause."""
        transaction = self._evaluate(
            self._record(drop="changelog"), pause_register=self._register()
        )
        self.assertEqual(transaction.verdict, "fail")
        self.assertEqual(transaction.exit_code, 1)

    def test_an_unreadable_register_is_an_error_not_an_absence_of_defects(self):
        """Test an unreadable register is an error not an absence of defects."""
    ...
    def test_an_entry_missing_a_required_field_is_an_error(self):
        ...
        register = _write_json(
            self.root,
            "release-pause-register.json",
            {"schema": "release-pause-register@1.0.0", "entries": [{"owner": _NAME}]},
        )
        with self.assertRaises(RecordError):
