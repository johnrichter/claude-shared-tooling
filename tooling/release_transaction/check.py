...
Usage:
    python3 tooling/release_transaction/check.py verify-release --record release.json
    python3 tooling/release_transaction/check.py changed --base origin/main
    python3 tooling/release_transaction/check.py render-manifest
        --version 1.2.3 --artifact linux-x86_64=dist/tool-linux-x86_64
    python3 tooling/release_transaction/check.py verify-manifest
        --manifest dist/manifest.json --public-key release-pubkey.pem
    python3 tooling/release_transaction/check.py provision --name tool --version 1.2.3
        --arch linux-x86_64 --cache-dir ~/.cache/tool --public-key release-pubkey.pem
"""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="release-transaction",
        description="Gate a release against the release-transaction contract.",
    )
    subcommands = parser.add_subparsers(dest="command", required=True)

    verify = subcommands.add_parser(
        "verify-release", help="Judge one release record against the contract."
    )
    ...
    verify.add_argument(
        ...
        type=Path,
        default=None,
        help="Contract file (default: the in-repo release-transaction contract).",
    )
    verify.add_argument(
        "--root",
        type=Path,
        default=None,
        help="Directory the record's paths resolve against (default: the working directory).",
    )
    verify.add_argument(
        "--pause-register",
        type=Path,
        default=None,
        help="Release-pause register to consult (default: the record's own, if it declares one).",
    )
    verify.add_argument("--json", action="store_true", help="Emit the result as JSON.")
    verify.set_defaults(handler=_verify_release)

    changed = subcommands.add_parser(
        "changed",
        help="Require every changed module to carry a version bump and a published release.",
    )
    changed.add_argument(
        "--base",
        default="origin/main",
        help="Base revision the change is measured from (default: origin/main).",
    )
    ...
    render = subcommands.add_parser(
        "render-manifest", help="Render a release manifest in one deterministic pass."
    )
    ...
    render.add_argument(
        ...
        default=[],
        metavar="ARCH=PATH",
        help="An artifact to record; repeatable.",
    )
    render.add_argument(
        "--out", type=Path, default=None, help="Write here instead of standard output."
    )
    render.set_defaults(handler=_render_manifest)

    verify_manifest = subcommands.add_parser(
        "verify-manifest", help="Authenticate a manifest against its detached signature."
    )
    verify_manifest.add_argument("--manifest", type=Path, required=True, help="Manifest file.")
    verify_manifest.add_argument(
        "--signature", type=Path, default=None, help="Detached signature (default: <manifest>.sig)."
    )
    verify_manifest.add_argument(
        "--public-key",
        type=Path,
        required=True,
        help="PEM public key the signature is verified against.",
    )
    verify_manifest.add_argument(
        "--artifact-dir",
        type=Path,
        default=None,
        help="Also check every listed artifact's bytes in this directory.",
    )
    verify_manifest.add_argument(
        "--version", default=None, help="Also require the manifest to state this version."
    )
    verify_manifest.set_defaults(handler=_verify_manifest)

    provision = subcommands.add_parser(
        "provision", help="Resolve a verified artifact by walking the provisioning ladder."
    )
    provision.add_argument(
        "--name", required=True, help="Module name, as the cache and release asset spell it."
    )
    ...
    provision.add_argument(
        "--cache-dir", type=Path, required=True, help="Version-keyed cache directory."
    )
    provision.add_argument(
        "--public-key", type=Path, required=True, help="PEM public key for the release signature."
    )
    provision.add_argument(
        "--base-url", default=None, help="Release host root (https:// or file://)."
    )
    provision.add_argument(
        "--frozen-dir", type=Path, default=None, help="Directory holding commit-frozen artifacts."
    )
...
def _changed(args: argparse.Namespace) -> int:
    ...
    for verdict in report.verdicts:
        ...
        print(
            f"release-transaction: {marker} {verdict.module.path} [{verdict.status}] - "
            f"{verdict.detail}"
        )
    if not report.verdicts:
        print(
            f"release-transaction: no released module changed between {report.base} and "
            f"{report.head}"
        )
    if report.failures:
        print(
            f"release-transaction: {len(report.failures)} changed module(s) carry no complete "
            f"release"
        )
    return report.exit_code
...
def _verify_manifest(args: argparse.Namespace) -> int:
    ...
    print(
        f"release-transaction: {verification.verdict.value} - {verification.code}: "
    ...
    if not provisioning.is_canonical(args.manifest.read_text(encoding="utf-8")):
        print(
            f"release-transaction: {args.manifest} is not the canonical rendering of its own "
            f"content -- it was not produced by a single deterministic pass"
        )
        return 1
    if args.version is not None and manifest.version != args.version:
        print(
            f"release-transaction: manifest states version {manifest.version}, not {args.version}"
        )
        return 1
    if args.artifact_dir is not None:
        for artifact in manifest.artifacts:
            ...
            if mismatch is not None:
                ...
                return 1
        print(
            f"release-transaction: {len(manifest.artifacts)} artifact(s) match their recorded "
            f"digests"
        )
    return 0
...
def _provision(args: argparse.Namespace) -> int:
    ...
    for attempt in result.attempts:
        print(
            f"release-transaction: rung {attempt.rung.value} {attempt.outcome} - {attempt.detail}",
            file=sys.stderr,
        )
