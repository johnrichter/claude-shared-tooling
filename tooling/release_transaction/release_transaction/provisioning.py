...
def _url_opener() -> urllib.request.OpenerDirector:
    """Return an opener limited to the schemes _ALLOWED_URL_SCHEMES permits (https and file).

    It carries no handler for http, ftp or any other scheme, so an asset URL outside that set
    raises URLError rather than being fetched. urllib.request.urlopen's default opener would
    honor those extra schemes for an artifact this process may go on to execute.
    """
...
def _install(source: Path, destination: Path, digest: str) -> Path:
    ...
    # Owner-only rwx: a provisioned binary in a per-user cache is run by the user who
    # provisioned it, so group/other need neither read nor execute (avoids a world-
    # readable/executable artifact — the permissive-mask concern B103 exists for).
