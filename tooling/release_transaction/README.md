...
```sh
python3 tooling/release_transaction/check.py verify-release --record release.json --root .
python3 tooling/release_transaction/check.py changed --base origin/main
python3 tooling/release_transaction/check.py render-manifest --version 1.2.3 --artifact linux-x86_64=dist/tool-linux-x86_64 --out dist/manifest.json
python3 tooling/release_transaction/check.py verify-manifest --manifest dist/manifest.json --public-key release/release-pubkey.pem --artifact-dir dist
python3 tooling/release_transaction/check.py provision --name tool --version 1.2.3 --arch linux-x86_64 --cache-dir "$XDG_CACHE_HOME/tool" --public-key release/release-pubkey.pem --base-url https://example.invalid/releases/download
```
...
- name: Changed modules carry a release
  run: python3 tooling/release_transaction/check.py changed --base "origin/${{ github.base_ref || 'main' }}"

- name: Release transaction is complete
  run: python3 tooling/release_transaction/check.py verify-release --record release.json --pause-register release-pause-register.json
```
