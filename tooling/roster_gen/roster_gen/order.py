...
def capability_order(ids: list[str], models: dict[str, Any]) -> list[str]:
    ...
    for fam in families:
        rank = family_rank(fam)
        if rank is not None:
            rank_by_family[fam] = rank

    ranked = sorted(rank_by_family, key=lambda fam: rank_by_family[fam], reverse=True)
    unranked = sorted(fam for fam in families if fam not in rank_by_family)
