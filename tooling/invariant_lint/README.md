...
Two of the invariant registry's five rungs (`schemas/invariant_registry`) carry a machine
...
the entry's fields are present and well-shaped — that is `schemas/invariant_registry/check.py`'s
...
completeness — `schemas/invariant_registry/check.py`. Rung 2's declared-vs-actual gate firing —
...
```sh
python3 tooling/invariant_lint/check.py
python3 tooling/invariant_lint/check.py --repo marketplace=../marketplace
python3 tooling/invariant_lint/check.py --require-roots   # every named repo must resolve
```
