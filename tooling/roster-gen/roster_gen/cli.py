"""Render every roster-derived output, or diff renderings against disk.

`generate` renders every roster-derived output in one pass; `check` diffs the
current renderings against what's on disk without writing.
"""
...
def generate(args: argparse.Namespace) -> int:
    """Render every target before writing any of them.

    A failure partway through rendering (e.g. the gate-allowlist narrowing guard) must not
    leave a partial write behind — a mix of regenerated and stale outputs is itself a drift,
    exactly what this generator exists to prevent.
    """
...
def check(args: argparse.Namespace) -> int:
    ...
    if failures:
        print(
            f"roster-gen: {len(failures)}/{len(TARGETS)} target(s) drifted or missing",
            file=sys.stderr,
        )
        return 1
    print(
        f"roster-gen: all {len(TARGETS)} targets match the roster-derived rendering",
        file=sys.stderr,
    )
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="roster-gen",
        description=(
            "Render every roster-derived output from the model roster, in one deterministic pass."
        ),
    )
    parser.add_argument(
        "--roster",
        type=Path,
        required=True,
        help="Path to model-roster.json, checked out at the pinned tag.",
    )
    parser.add_argument(
        "--tag",
        required=True,
        help="Tag/ref the roster was resolved at — named in every output's generated-by header.",
    )
    parser.add_argument(
        "--ai-shared-lib-root", type=Path, required=True, help="ai-shared-lib repo root."
    )
    parser.add_argument(
        "--marketplace-root", type=Path, required=True, help="marketplace repo root."
    )
    ...
    sub.add_parser(
        "check",
        help=(
            "Render every target in memory and diff against disk; exit 1 on drift "
            "or an absent target."
        ),
