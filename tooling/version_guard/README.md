...
```sh
python3 tooling/version_guard/check.py check-tag go/git/v1.2.0
python3 tooling/version_guard/check.py check-deps
python3 tooling/version_guard/check.py commands --module go/git --version 1.2.0
```
...
- name: Tag conforms to SC-VERSIONING
  ...
  run: python3 tooling/version_guard/check.py check-tag "${GITHUB_REF_NAME}"

- name: No Rust path/relative dependencies
  run: python3 tooling/version_guard/check.py check-deps
```
