...
```sh
python3 tooling/dist_guard/check.py scan --root .
python3 tooling/dist_guard/check.py generate --root .
python3 tooling/dist_guard/check.py check --root .
```
...
- name: No committed binaries outside the SC-DISTRIBUTION allowlist
  run: python3 tooling/dist_guard/check.py scan --root .
- name: Allowlist is current with its producer
  run: python3 tooling/dist_guard/check.py check --root .
```
