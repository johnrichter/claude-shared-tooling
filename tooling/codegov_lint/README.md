...
- **ruff** (pinned in `requirements.txt`), pydocstyle rules (`D`) in Google convention, is
  ...
  ```sh
  python3 -m venv .venv && .venv/bin/pip install -r tooling/codegov_lint/requirements.txt
  ```
...
```sh
python3 tooling/codegov_lint/check.py --diff <ref>   # files changed since <ref> — CI mode
python3 tooling/codegov_lint/check.py --files a.py b.go
python3 tooling/codegov_lint/check.py --all           # every tracked source file — audit only
```
