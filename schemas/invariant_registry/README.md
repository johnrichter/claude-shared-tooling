...
|---|---|---|---|
| 1 | impossible in code — a fail-fast where the value is produced | `fail_fast_symbol` (`file:symbol`) | `ai-shared-lib/tooling/invariant_lint` resolves the symbol |
...
| 3 | a check in the shipped suite | `test_id` | `ai-shared-lib/tooling/invariant_lint` asserts it exists and runs |
...
```sh
python3 -m venv .venv && .venv/bin/pip install -r schemas/invariant_registry/requirements.txt
.venv/bin/python schemas/invariant_registry/check.py                          # this checkout only
.venv/bin/python schemas/invariant_registry/check.py --repo marketplace=../marketplace
.venv/bin/python schemas/invariant_registry/check.py --require-roots          # every root must resolve
```
...
A language-agnostic contract module: it releases as `schemas/invariant_registry/vX.Y.Z` — the tag prefix equals the module's path from the repository root, with no language segment. The schema carries its own `version`/`$id`, so a consumer pins a contract version rather than a checkout.
