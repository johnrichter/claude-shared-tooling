...
**Historical ledger — superseded pack name, current profile unchanged.** This file was authored against the original `core@1` + `frontmatter-psa-apm.pack.json` pair (M2.P1.T1). `psa-apm@1` was later retired: its generic vocabulary split out into `frontmatter-default.pack.json` (`default@1`) and the report bundle into `frontmatter-reports.pack.json` (`reports@1`); its Datadog/psa-apm-specific vocabulary moved to a private repo as `datadog-psa@1`. `core@1` itself was later superseded by `core@2` (see `frontmatter-core.schema.json`'s `description`) — a breaking change unrelated to the pack split. Every `psa-apm@1` mention below is preserved as-written — it is what was true when M2.P2.T2 captured its parity baseline — and is NOT edited to track the current pack names. For the current state, see `schemas/frontmatter/README.md` (profile/pack table) — the mechanisms this ledger documents (D1-D6) carried forward into `default@1`/`reports@1` unchanged; only the file that held them moved.
...
## D6 — real-calendar `period` validation added (Rust-only, verdict-changing)

- **schema.py / live Python gate**: `PERIOD_RE` is shape-only (`^\d{4}-\d{2}-\d{2}/\d{4}-\d{2}-\d{2}$`); it accepts any digit string in that shape, including a calendar-impossible endpoint (`2026-13-45`, `2026-02-30`, a non-leap `2025-02-29`) or an inverted interval (`end/start`). Deliberately left as-is — the Python gate retires into navigator (M2.P2.T3) rather than being extended.
- **Declarative + Rust (`frontmatter::validate`)**: after the existing shape regex passes, `report_period_format_phase` additionally parses each endpoint as a real `time::Date` (rejecting a leading sign) and requires `start <= end` by `Date` comparison. A calendar-impossible endpoint or an inverted interval now emits `INVALID_PERIOD_FORMAT`, with a message distinguishing the calendar failure ("... is not a real YYYY-MM-DD/YYYY-MM-DD calendar interval") from the pre-existing shape failure ("... is not YYYY-MM-DD/YYYY-MM-DD").
- **Authority**: task spec (Time-B) — closes the previously-documented gap (facetquery's `query_sdet_adversarial.rs` KNOWN LIMITATION / Decision 10's open minor) where a shape-valid-but-inverted period silently passed validation while being unmatchable by any query.
- **Verdict impact**: a period with a calendar-impossible or inverted endpoint now emits `INVALID_PERIOD_FORMAT` in Rust where the live Python gate emits nothing. This is a NEW intentional Rust-vs-Python delta, not a regression — the SC6' cutover corpus (`tests/cutover/fixtures/`, `expected_verdicts.json`) contains no calendar-invalid or inverted `period:` value (its one shape-invalid fixture, `period:bad-format`, fails the pre-existing shape gate and never reaches the calendar gate), so the differential stays green with no normalization change needed.

## Non-divergences (faithful ports — see report checklist)

The five tag-rule codes (`MISSING_REQUIRED_TAG`, `MULTIPLE_SINGLE_VALUE_TAGS`, `ORPHAN_NAMESPACE_TAG`, `REPORT_ONLY_TAG_MISUSED`, `INVALID_PERIOD_FORMAT`), their fields, message templates, cascade ORDER, the parent map (feature→product,suite; product→suite), report rules (required source+period; cadence-defaults; period regex), and the exempt taxonomy (filenames/dir-components/path-globs) are ported verbatim with source line citations. No behavior change there — EXCEPT `INVALID_PERIOD_FORMAT`'s Rust-side calendar-validity gate, a reviewed addition on top of the verbatim shape-regex port; see D6.
