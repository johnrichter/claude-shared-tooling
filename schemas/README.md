...
- **`invariant_registry/`** — the source of record for every enforcement invariant: its rung on the enforcement ladder, its fail direction and the blast radius that chose it, and the per-rung field a named consumer verifies. Ships its own lint for the properties that are internal to the registry — validity, no restatement, gate completeness. See its `README.md`.
