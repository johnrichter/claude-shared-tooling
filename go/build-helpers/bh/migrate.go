...
// This file implements `migrate-project`: upgrading an in-flight v1 project's parsed
...
//   - execution.json schema_version stamp — absent on every pre-v2 file; stamped to
...
//   - id + name on every entity — v1 tasks carry no `name`; v2 requires a short human name
...
//     project. All are nil-safe pointers / omitempty: an absent field on load is the tolerated
...
// doNotDispatchPattern matches the literal "do not dispatch" hand-note convention — the
// exact phrasing an old stopgap used to keep a task off subagent dispatch,
...
// migratePlanInPlace applies the plan-side v2 deltas: backfill every entity name and map any
...
func migratePlanInPlace(p *Plan, rep *MigrateReport) {
	for mi := range p.Milestones {
		...
		if strings.TrimSpace(m.Name) == "" {
			n := "Milestone " + m.ID
			rep.Changes = append(rep.Changes, MigrateChange{Target: "plan", ID: m.ID, Field: "name", From: "", To: n, Note: "milestone name backfilled from id"})
			m.Name = n
		}
		for pi := range m.Phases {
			...
			if strings.TrimSpace(ph.Name) == "" {
				n := "Phase " + ph.ID
				rep.Changes = append(rep.Changes, MigrateChange{Target: "plan", ID: ph.ID, Field: "name", From: "", To: n, Note: "phase name backfilled from id"})
				ph.Name = n
			}
			for ti := range ph.Tasks {
				...
				if strings.TrimSpace(t.Name) == "" {
					...
					rep.Changes = append(rep.Changes, MigrateChange{Target: "plan", ID: t.ID, Field: "name", From: "", To: n, Note: "task name backfilled from summary"})
					t.Name = n
				}
...
// migrateOrchestratorOnly promotes the legacy "do not dispatch" hand-note convention into the
// structural orchestrator_only field: before that field existed, the
// only way to keep a measurement/verdict-gate task off subagent dispatch was a plan-level risk
// marked forces_pause whose risk/mitigation text carried the "do not dispatch <task id>" note. A
// task named by such a note and not already orchestrator_only is
		...
		for pi := range p.Milestones[mi].Phases {
			for ti := range p.Milestones[mi].Phases[pi].Tasks {
				...
				if named[t.ID] && !t.OrchestratorOnly {
					rep.Changes = append(rep.Changes, MigrateChange{Target: "plan", ID: t.ID, Field: "orchestrator_only", From: "false", To: "true", Note: "promoted from a forces_pause hand-note naming this task by id (design-declared do-not-dispatch)"})
...
func migrateExecInPlace(ex *ExecState, rep *MigrateReport) error {
	...
	if strings.TrimSpace(ex.Name) == "" {
		...
		rep.Changes = append(rep.Changes, MigrateChange{Target: "execution", Field: "name", From: "", To: n, Note: "execution name backfilled"})
		ex.Name = n
	}
...
// nameFromSummary derives a short task name from the summary, deterministically: the first
