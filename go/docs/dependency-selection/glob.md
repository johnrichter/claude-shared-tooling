...
Sensitivity: **public** · Owner: **public** · Kind: **dependency-selection decision doc**
...
**Application site:** `go/bh/surface.go` — the shared lib's file-surface verifier. It currently matches allow/deny path patterns with Go's `filepath.Match`/`fs.Glob`, neither of which supports `**` (globstar). Adopting doublestar here (M2.P2.T1) is what fixes the gap.
...
- **Tracking requirement (M2.P2.T1 consuming change, not this spike):** record in `ai-shared-lib-public/LICENSE-3rdparty.csv` (component `doublestar`, origin `https://github.com/bmatcuk/doublestar`, license `MIT`, copyright `Bob Matcuk`). If `go/bh` ships inside the repo's distributed signed binary, the MIT permission/license text must also travel with that binary — reproduce it in `THIRD-PARTY-LICENSES.md` per the repo's existing attribution practice. M2.P2.T1 must confirm the distribution status of the Go surface and satisfy whichever obligation applies.

## Consuming this decision

- M2.P2.T1 adopts `github.com/bmatcuk/doublestar/v4 v4.10.0` in `go/bh/surface.go`, adds the `require` line + `go.sum` entries above to `go/go.mod`/`go/go.sum`, and adds the `LICENSE-3rdparty.csv` row (plus the `THIRD-PARTY-LICENSES.md` entry if distributed).
- No other capability spike (JSON-schema validation, diffing, semver, CLI parsing) is decided by this doc — each gets its own M2.P1 spike per `SC-DEPVET`.
